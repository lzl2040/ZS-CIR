...
import prompts_img
available_prompts = [f'prompts.{x}' for x in prompts_objects.__dict__.keys() if '__' not in x]
prompt_object = prompts_objects.mllm_objects_prompt_CoT
...
def fiq_generate_shared_concept(dress_type, messages, processor, model, output_json):
    with open(f'/data/tangwenyue/Code/ZS-CIR/Qwen2_5_VL/CoTMR/tar.cap.{dress_type}.val.json') as f:
    ...
    for data in tqdm(triplets):
        ...
        try:
            objects_json = re.sub(r"^```json\s*|\s*```$", "", output_text[0].strip())
            objects_dict = json.loads(objects_json)
            existent_objects = objects_dict["Existent Object"]
            unexist_objects = objects_dict["Unexist Object"]
        except Exception as e:
            existent_objects = "<PARSE_ERROR>"
            unexist_objects = "<PARSE_ERROR>"
            print(f"Warning: Failed to parse output: {output_text[0]}")
        data['existent_objects'] = existent_objects
        data['unexist_objects'] = unexist_objects
...
def circo_generate_shared_concept(messages, processor, model, output_json):
    ...
    with open('/data/tangwenyue/Code/ZS-CIR/Qwen2_5_VL/CoTMR/tar.val.json') as f:
